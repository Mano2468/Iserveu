(function() {
    'use strict';
    var ApiAddBenefactory = function($http, $location,API) {
        var Benefactory = {};
        var CardNo;
        var securitykey;
        var Mnumber;
        ...
        Benefactory.postbene = function(Type, Choose, BeneName, Benemobile,
            ...
            if (!(CardNo == null)) {
                return $http({
                    method: 'POST',
                    url: API+'dcashaddbeneficiary.json',
    ...
    ApiAddBenefactory.$inject = ["$http", "$location","API"];
    angular.module("app").factory('ApiAddBenefactory', ApiAddBenefactory);
}());
